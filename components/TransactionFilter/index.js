...
const TransactionFilter = ({ transactions, onFilteredTransactions, onFilterChange }) => {
  ...
  const filterOptions = useMemo(() => {
    ...
    const transactionTypes = [...new Set(transactions.map(t => t.Transaction))].map(type => ({
      value: type,
      label: type
    }));
    ...
    const categories = [...new Set(transactions.map(t => t["Category or Account"]))]
      .sort((a, b) => a.localeCompare(b))
      .map(category => ({
        value: category,
        label: category
  ...
  const filteredTransactions = useMemo(() => {
    ...
    return transactions.filter(transaction => {
      ...
      if (filters.transactionType.length > 0 && !filters.transactionType.includes(transaction.Transaction)) {
        return false;
      }

      // Filter by account
      if (filters.account.length > 0 && !filters.account.includes(transaction.Account)) {
        return false;
      }

      // Filter by category
      if (filters.category.length > 0 && !filters.category.includes(transaction["Category or Account"])) {
        return false;
      }

      // Filter by note
      if (filters.note && !transaction.Note?.toLowerCase().includes(filters.note.toLowerCase())) {
        return false;
      }
  ...
  const hasActiveFilters = () => {
    return filters.transactionType.length > 0 ||
        ...
        <div className="flex items-center space-x-2">
          ...
          {hasActiveFilters() && (
            <span className="px-2 py-1 text-xs font-medium bg-blue-100 text-blue-800 rounded-full">
              {filteredTransactions.length} of {transactions?.length || 0}
            </span>
          )}
        </div>
