...
export const processBudgetData = (rawData, month) => {
    ...
    const spendingMonthly = extractMonthlyData(
        parsedSpending,
        month,
        "Spending"
    );
    const earningMonthly = extractMonthlyData(parsedEarning, month, "Earning");
    const transferMonthly = extractMonthlyData(
        parsedTransfer,
        month,
        "Transfer"
    );
    const spendingTFMonthly = extractMonthlyData(
        parsedSpendingTF,
        month,
        "SpendingTF"
    );
    ...
    return {
        ...
        summary: {
            totalEarning: Object.values(earningMonthly).reduce(
                (sum, item) => sum + item.actual,
                0
            ),
            totalSpending: Object.values(spendingMonthly).reduce(
                (sum, item) => sum + item.actual,
                0
            ),
            totalTransfer: Object.values(transferMonthly).reduce(
                (sum, item) => sum + item.budget,
                0
            ),
            totalSpendingTF: Object.values(spendingTFMonthly).reduce(
                (sum, item) => sum + item.actual,
                0
            ),
        },
...
const parseCSV = (data, sheetType = "") => {
    ...
    const parsedData = result.data.filter((row) => {
        const hasCategory =
            row[firstColumnName] && row[firstColumnName].trim() !== "";
...
const getFirstColumnName = (sheetType, fields) => {
    ...
    if (sheetType === "SpendingTF") {
        return (
            fields.find(
                (header) =>
                    header.includes("SPENDINGTF") ||
                    header.startsWith("SPENDINGTF")
            ) || fields[0]
        );
    }
